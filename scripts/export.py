...
def export_html(df: pd.DataFrame, path: str = "output/draft_board.html") -> None:
    ...
    for _, row in df.iterrows():
        ...
        best_pos   = row.get("Best_Pos", pos)
        eligibility = row.get("Eligibility", pos)
        color      = pos_colors.get(best_pos, pos_colors.get(pos, "#6b7280"))
        adp        = row.get("ADP", 999)
        ...
        # Build eligibility badge — show all positions, highlight best
        elig_parts = eligibility.split("/") if eligibility else [pos]
        elig_html  = ""
        for p in elig_parts:
            p = p.strip()
            if not p:
                continue
            c = pos_colors.get(p, "#6b7280")
            if p == best_pos:
                elig_html += f'<span class="pos-badge" style="background:{c}22;color:{c};border:1px solid {c}44">{p}</span>'
            else:
                elig_html += f'<span class="pos-badge secondary" style="background:{c}11;color:{c}88;border:1px solid {c}22">{p}</span>'

        rows_html += f"""
        <tr data-pos="{best_pos}">
            <td class="rank-cell">#{rank}</td>
            <td class="name-cell">
                <span class="player-name">{name}</span>
            </td>
            <td class="elig-cell">{elig_html}</td>
